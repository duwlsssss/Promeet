...
const SchedulePage = () => {
  ...
  const { mutateAsync: createPromiseAsync, isPending: isCreatePromisePending } = useCreatePromise();
  ...
  const handleCreatePromiseBtnClick = async () => {
    ...
    try {
      const res = await createPromiseAsync({
        // mutateAsync로 이름을 바꿔서 가져오세요
        creatorId: userId,
        promiseName: name,
  ...
  const handleTimeTableChange = (selected) => {
    ...
    const newAvailableTimes = availableTimes.map((item, dayIdx) => {
      ...
      const dayArr = Array.from({ length: 24 }, (_, h) =>
        Array.from({ length: 4 }, (_, q) => selected[h]?.[dayIdx]?.[q] || false),
      );
