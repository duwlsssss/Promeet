...
export const getPlaceLike = async (promiseId, placeId, userId) => {
  const { data } = await axiosInstance.get(`/promise/likes`, {
...
export const getPromiseData = async (promiseId, userId) => {
  const { data } = await axiosInstance.get(`/promise/${promiseId}`, {
    params: { userId },
  });
  return data;
};
...
export const getPromiseSummaryData = async (promiseId) => {
  const { data } = await axiosInstance.get(`/promise/${promiseId}/summary`);
  return data;
};

// 루트 가져오기
export const getRoutes = async (promiseId) => {
  const { data } = await axiosInstance.get(`/promise/${promiseId}/routes`);
  return data;
};
